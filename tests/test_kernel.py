...
class OpenAIKernelTests(jupyter_kernel_test.KernelTests):
    ...
    def test_openai_no_api_key(self):
        ...
        self.assertEqual(
            ...
            "<class 'openai.error.AuthenticationError'>",
        )
        assert output_msgs[0]["content"]["evalue"].startswith("No API key provided")
        assert output_msgs[0]["content"]["traceback"][0].startswith(
            "No OpenAI API key provided, set your API key by using the 'magic' "
            "commands '%api_key API_KEY' or '%api_key_path PATH_TO_API_KEY', creating "
            "a .openai_api_key file in your home directory, or by setting the "
    ...
    def test_openai_set_api_key_magic(self):
        ...
        self.assertEqual(
            output_msgs[0]["content"]["data"]["text/plain"], "'/path/to/sekrit'"
        )
    ...
    def test_openai_set_variable(self):
        ...
        reply, output_msgs = self.execute_helper(
            code="%set system_prompt aliens have taken over earth"
        )
        reply, output_msgs = self.execute_helper(code="%get system_prompt")
        self.assertEqual(output_msgs[0]["header"]["msg_type"], "execute_result")
        self.assertEqual(
